...
export default function ComptesPage() {
  ...
  const [form, setForm] = useState({ clientId: "", type: "COURANT", soldeInitial: 0, projetNom: "" });

  async function load() {
    const [cRes, clRes] = await Promise.all([
      fetch(`/api/comptes?q=${search}&type=${typeFilter}`).then((r) => r.json()),
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button className="btn btn-primary" onClick={() => setModalOpen(true)}>+ Ouvrir un compte</button>
      </div>
      <div className="card flex gap-3">
        ...
        <input placeholder="Rechercher..." className="input max-w-sm" value={search} onChange={(e) => setSearch(e.target.value)} />
      </div>
          ...
          <tbody>
            {comptes.map((c) => (
              <tr key={c.id} className="border-t border-gray-100">
                ...
                <td className="font-medium">{formatMoney(c.solde)}</td>
                <td>{formatDate(c.dateOuverture)}</td>
                <td><span className="badge bg-green-100 text-green-700">{c.statut}</span></td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <form onSubmit={saveCompte} className="space-y-3">
          ...
          {form.type === "PROJET" && (
            <div>
              ...
              <input className="input" value={form.projetNom} onChange={(e) => setForm({ ...form, projetNom: e.target.value })} />
            </div>
          )}
          <div>
            <label className="text-sm text-gray-600">Dépôt initial</label>
            <input type="number" className="input" value={form.soldeInitial} onChange={(e) => setForm({ ...form, soldeInitial: Number(e.target.value) })} />
          </div>
